...
class CondBlock(nn.Module):
    ...
    def forward(self, x, c, y):
        ...
        shift_y, scale_y, gate_y, shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(
            c).chunk(9, dim=1)
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, patch_size, frame_patch_size, out_channels):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.linear = nn.Linear(hidden_size, patch_size * patch_size * frame_patch_size * out_channels, bias=True)
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
...
class FullAttentionVideoDiT(nn.Module):
    """
    ...
    def __init__(
        ...
        self.x_embedder = PatchEmbedder(input_size, patch_size, frame_size, frame_patch_size, in_channels, hidden_size,
                                        bias=True)
        ...
        self.blocks = nn.ModuleList([
            CondBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer(hidden_size, patch_size, frame_patch_size, self.out_channels)
    ...
    def decouple(self, t, y):
        indices = range(0, t.size(0), self.frame_size)
        t = t[indices]
    ...
    def unpatchify(self, x):
        """
        x: (B, T, patch_size**2 * frame_patch_size * C)
        imgs: (B * F, C, H, W)
        """
        c = self.out_channels
        fp = self.frame_patch_size
        p = self.patch_size
        f = self.x_embedder.grid_size[0]
        h = w = self.x_embedder.grid_size[1]

        x = x.reshape(shape=(x.shape[0], f, h, w, fp, p, p, c))
        x = torch.einsum('nfhwtpqc->nftchpwq', x)
        x = x.reshape(x.shape[0], f * fp, c, h * p, h * p)  # (B, F, C, H, W)
        imgs = x.flatten(0, 1)
        return imgs
    ...
    def forward(self, x, t, y):
        ...
        for block in self.blocks:
            x = block(x, t, y)  # (B, T, D)
        x = self.final_layer(x, t)  # (B, T, patch_size ** 2 * frame_patch_size * out_channels)
...
def DiT_XL_2(**kwargs):
    return FullAttentionVideoDiT(depth=28, frame_size=16, hidden_size=1152, frame_patch_size=2, patch_size=2,
                                 num_heads=16, **kwargs)


def DiT_L_4(**kwargs):
    return FullAttentionVideoDiT(depth=24, frame_size=16, hidden_size=1024, frame_patch_size=4, patch_size=4,
                                 num_heads=16, **kwargs)
def DiT_B_4(**kwargs):
    return FullAttentionVideoDiT(depth=12, frame_size=8, hidden_size=768, frame_patch_size=1, patch_size=4, num_heads=12, **kwargs)
